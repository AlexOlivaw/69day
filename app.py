...
class BlogPost(Base, db.Model):
    ...
    users = relationship("User", back_populates="blog_posts")
...
class User(Base, UserMixin, db.Model):
    ...
    blog_post = relationship("BlogPost", back_populates="users")
    blog_post_id = db.Column(db.Integer, db.ForeignKey(blog_post))
